...
class DeviceBase(Updatable):
    ...
    async def doUpdate(self):
        ...
        if "Rooms" not in self.document:
            self.document["Rooms"] = {}
        if "Channels" not in self.document:
            self.document["Channels"] = {}
        if "Schedule" not in self.document:
            self.document["Schedule"] = {}
        if "DeletedChannels" not in self.document:
            self.document["DeletedChannels"] = []
        if "Info" not in self.document:
            self.document["Info"] = {}
        if "Pending" not in self.document:
